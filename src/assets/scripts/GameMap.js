...
export class GameMap extends AcGameObject {
    constructor(ctx, parent) {
        ...
        this.snakes = [
            new Snake({id: 0, color: "#F94847", r: this.rows - 2, c: 1}, this),
            new Snake({id: 1, color: "#41BA83", r: 1, c: this.cols - 2}, this),
        ];
    ...
    check_vaild(cell){
        ...
        for(const snake of this.snakes){
            ...
            if(!snake.check_tail_increase()){   // 蛇尾会前进
                k --;
            }
            for(let  i= 0;i<k;i++){
    ...
    render() {
        const color_even = "#CFDAED", color_odd = "#DDE1EA";
        for (let r = 0; r < this.rows; r++) {
            for (let c = 0; c < this.cols; c++) {
                if ((r + c) % 2 === 0) {
